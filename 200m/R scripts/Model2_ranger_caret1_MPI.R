...
calculate_predictions <- function(idx_fold=NULL,sub_train=NULL, sub_test=NULL) {
  ...
  ##############################################################################
    ...
    rangerMetrics=calculateMMetrics(y_true=sub_test$resid,
...
for (i_test in 1:nsplit) {
  sub_test = read.fst(paste0(current_year,"_pred_res_200.RData.fst"), 
  ...
  for (i_file in 1:length(files_to_predict)) {
    ...
    updated_DMdf = read.fst(files_to_predict[i_file],
                            from=length_to_skip + 1,
                            to=length_to_skip + length_sub_test,
  ...
  ranger_Roberts1 <- do.call(c, cpred)
  all_predict_ranger1 = cbind(predict_ranger1 = ranger_Roberts1, dplyr::select(sub_test, c(x,y,time)))
  save(all_predict_ranger1, file = paste0("all_predict_ranger1_", current_year, "_", i_test, ".RData"))
