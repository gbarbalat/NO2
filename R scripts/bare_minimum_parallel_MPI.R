...
name=list_param$name
 ...
 concat_raster=france_grid
  ...
  if (mosaicking) {
      all_mosaic=lapply(paste0(here_data_pred,files_mosaic),read_stars,sub=sub,driver=NULL,proxy=FALSE)
      file_read=st_as_stars(do.call(st_mosaic,all_mosaic))
      #file_read<-st_set_crs(file_read,crs_num)
 ...
 if (take_part_raster) {
    ...
    concat_raster = st_set_dimensions(file_read,
                                      which="Times", #replace that dimension ...
                                      names = "time", #...by that one
                                      values = 1:days_in_total)
  ...
  for (i in  1:days_in_total){
    ...
    if(monitors) {
      ...
      file_read=st_rasterize(file_sf[,i],france_grid)#dx = 1000, dy = 1000)
