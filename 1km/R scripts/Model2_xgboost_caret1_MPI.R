...
CV_predict_xgb=predict.train(hyper_params,
                                 dplyr::select(sub_test,all_of(predictors))
)

#importance
xgbImportance=varImp(hyper_params)

#evaluate predictions
    if (!!length(sub_test$no2_monitors)) {
    xgbMetrics=calculateMMetrics(y_true=sub_test$no2_monitors,
                                    y_hat=matrix(CV_predict_xgb,ncol=1))
    } else {xgbMetrics=NULL}

    #save predictions
    predictions_NO2_xgb=CV_predict_xgb

return(list(xgbImportance,xgbMetrics,predictions_NO2_xgb,hyper_params,preproc_f))
  
}#end of function
############################################################################
############################################################################
nc=detectCores()
xgb_results=mclapply(1:nfolds, calculate_predictions,mc.cores=nc)
save(xgb_results,file=paste0("all_predict_xgb_folds_",current_year,".RData"))
...
xgb_Roberts1<- do.call (c, cpred ) # mc
all_predict_xgb1=cbind(predict_xgb1=xgb_Roberts1,dplyr::select(sub_test,c(X,Y,time)))
save(all_predict_xgb1,file=paste0("all_predict_xgb1_month",i_test,"_",current_year,".RData"))

rm(all_predict_xgb1,xgb_Roberts1,sub_test,updated_DMdf,updatedDM_raster)
}
...
print(idx_fold)
(all_metrics_folds[[idx_fold]]=xgb_results[[idx_fold]][[2]])
all_importance_folds[[idx_fold]]=xgb_results[[idx_fold]][[1]]

sub_test=df[idx_out[[idx_fold]],]
all_prediction=c(all_prediction,xgb_results[[idx_fold]][[3]])
...
save(global_metrics,all_metrics_folds, all_importance_folds, 
     ...
     file=paste0("all_metrics_xgb_",current_year,".RData")
)
}
