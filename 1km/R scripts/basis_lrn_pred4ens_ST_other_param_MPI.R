...
for (idx_fold in (fold_to_repare:fold_to_repare)) {#  outer folds 1:nfolds
  ...
  for (i in 1:length(idx_folds_sub)) {#inner folds
    ...
    find_sub_train_test=function(df_sub) {
      ...
      predictors=df_sub %>% dplyr::select(!contains(c("omi","cams","no2_monitors","spacevar","spacetimevar","period","spaceperiodvar"))) %>% 
        colnames()
