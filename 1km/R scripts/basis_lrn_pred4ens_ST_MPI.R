...
#for (idx_fold in (10:nfolds)) {#  outer folds 1:nfolds
  ...
  for (i in 1:length(idx_folds_sub)) {#inner folds
    ...
    find_sub_train_test=function(df_sub) {
      ...
      predictors=df_sub %>% dplyr::select(!contains(c("omi","no2_monitors","spacevar","spacetimevar","period","spaceperiodvar"))) %>% 
        colnames()
    ...
    #ranger
    ##########
    hyper_params_ranger= caret::train(y=sub_train$no2_monitors,
    ...
    #XGB
    ##########
    hyper_params_XGB = caret::train(y=sub_train$no2_monitors,
                                    x=dplyr::select(sub_train,all_of(predictors)),
                                    method = "xgbTree",
                                    preProcess=NULL,
                                    trControl = fitControl,
    ...
    #catboost
    ##########
    hyper_params_catboost = caret::train(y=sub_train$no2_monitors,
                                    x=dplyr::select(sub_train,all_of(predictors)),
    ...
    all_ytrue_sub=c(all_ytrue_sub,sub_test$no2_monitors)	
...
#                                                  "_POST_basislrn.RData")
#))
load(file=paste0(path_to,paste0("holdout_folds_",1,"_",current_year,"_POST_basislrn.RData")))
tmp[[1]]=tmp_tmp;rm(tmp_tmp)

load(file=paste0(path_to,paste0("holdout_folds_",2,"_",current_year,"_POST_basislrn.RData")))
tmp[[2]]=tmp_tmp;rm(tmp_tmp)

load(file=paste0(path_to,paste0("holdout_folds_",3,"_",current_year,"_POST_basislrn.RData")))
tmp[[3]]=tmp_tmp;rm(tmp_tmp)

load(file=paste0(path_to,paste0("holdout_folds_",4,"_",current_year,"_POST_basislrn.RData")))
tmp[[4]]=tmp_tmp;rm(tmp_tmp)

load(file=paste0(path_to,paste0("holdout_folds_",5,"_",current_year,"_POST_basislrn.RData")))
tmp[[5]]=tmp_tmp;rm(tmp_tmp)

load(file=paste0(path_to,paste0("holdout_folds_",6,"_",current_year,"_POST_basislrn.RData")))
tmp[[6]]=tmp_tmp;rm(tmp_tmp)

load(file=paste0(path_to,paste0("holdout_folds_",7,"_",current_year,"_POST_basislrn.RData")))
tmp[[7]]=tmp_tmp;rm(tmp_tmp)

load(file=paste0(path_to,paste0("holdout_folds_",8,"_",current_year,"_POST_basislrn.RData")))
tmp[[8]]=tmp_tmp;rm(tmp_tmp)

load(file=paste0(path_to,paste0("holdout_folds_",9,"_",current_year,"_POST_basislrn.RData")))
tmp[[9]]=tmp_tmp;rm(tmp_tmp)

load(file=paste0(path_to,paste0("holdout_folds_",10,"_",current_year,"_POST_basislrn.RData")))
tmp[[10]]=tmp_tmp;rm(tmp_tmp)
