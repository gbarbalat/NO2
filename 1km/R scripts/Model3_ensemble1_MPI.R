...
calculate_predictions <- function(idx_fold) {
  ...
  ens_DB_test=data.frame(all_predRanger_sub=ens_DB_test_ranger,
                         ...
                         all_ytrue_sub=df[idx_out[[idx_fold]],"no2_monitors"]
  )
...
save(ens_results,file=paste0("all_predict_ens_folds",current_year,".RData"))
...
for (idx_fold in 1:nfolds) {
  ...
  y_true=c(y_true,ens_DB_test$no2_monitors)
...
#X,Y,time
#HERE CHANGE
#load(file=paste0(path_to,"data4_Model2_5.RData"))
...
load(paste0(path_to,"all_predict_ranger1_month",i_test,"_",current_year,".RData"))#all predictions 1 & 2 correspond to Roberts 1 and 2
load(paste0(path_to,"all_predict_xgb1_month",i_test,"_",current_year,".RData"))#all predictions 1 & 2 correspond to Roberts 1 and 2
load(paste0(path_to,"all_predict_catboost1_month",i_test,"_",current_year,".RData"))#all predictions 1 & 2 correspond to Roberts 1 and 2
...
save(all_predict_ens1, file=paste0("all_predict_ens1_month",i_test,"_",current_year,".RData"))
