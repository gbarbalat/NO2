...
which_folds_type="STBlMo_5F"#"STBlMo_5F"
...
calculate_predictions <- function(idx_fold=NULL,sub_train=NULL, sub_test=NULL) {
  ...
  ##############################################################################
    ...
    CV_predict_ranger=predict.train(hyper_params,
                                 dplyr::select(sub_test,all_of(predictors))
    )

    #importance
    rangerImportance=varImp(hyper_params)

    #evaluate predictions
    if (!!length(sub_test$no2_monitors)) {
    rangerMetrics=calculateMMetrics(y_true=sub_test$no2_monitors,
                                    y_hat=matrix(CV_predict_ranger,ncol=1))
    } else {rangerMetrics=NULL}

    #save predictions
    predictions_NO2_ranger=CV_predict_ranger

return(list(rangerImportance,rangerMetrics,predictions_NO2_ranger,hyper_params,preproc_f))
  
}#end of function
############################################################################
############################################################################
nc=detectCores()
ranger_results=mclapply(1:nfolds, calculate_predictions,mc.cores=nc)
#ranger_results=lapply(1:nfolds, calculate_predictions)
save(ranger_results,file=paste0("all_predict_ranger_folds_",current_year,".RData"))
...
data4_Model2= data4_Model2 %>% as.data.frame() %>%
			      ...
			      mutate(time_day = time %% 7)  %>% 
              		      mutate(time_month = month(as.Date(time, origin = paste0(current_year-1,"-12-31")))) 
			      #%>% mutate(X=x,Y=y) %>% select(-c("x","y","cams"))
...
sub_test = data4_Model2[[i_test]]
	...
	for (i in 3:40) {#DO NOT take OMI, DO NOT TAKE CAMS, so start from 3
...
ranger_Roberts1<- do.call (c, cpred ) # mc
all_predict_ranger1=cbind(predict_ranger1=ranger_Roberts1,dplyr::select(sub_test,c(X,Y,time)))
save(all_predict_ranger1,file=paste0("all_predict_ranger1_month",i_test,"_",current_year,".RData"))

rm(all_predict_ranger1,ranger_Roberts1,sub_test,updated_DMdf,updatedDM_raster)
}
...
print(idx_fold)
(all_metrics_folds[[idx_fold]]=ranger_results[[idx_fold]][[2]])
all_importance_folds[[idx_fold]]=ranger_results[[idx_fold]][[1]]

sub_test=df[idx_out[[idx_fold]],]
all_prediction=c(all_prediction,ranger_results[[idx_fold]][[3]])
...
save(global_metrics,all_metrics_folds, all_importance_folds, 
     ...
     file=paste0("all_metrics_ranger_",current_year,".RData")
)
}
